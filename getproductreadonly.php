...
/* 
 * displays imei. locator id and name and sim in read only mode based on product model
 */
    ...
    while($sql1->fetch()){
        ...
        echo '<input type="text" value="'.$simnumber.'" id="sim'.$i.'" class="noedit" readonly></input>';
        echo '<input type="hidden" value="'.$productmodelsim.'" id="idsim'.$i.'"></input>';
        
        echo '<label for="locatorid'.$i.'">Locator ID'.$i.':</label>';
        echo '<input type="text" value="'.$locatorid.'" id="locatorid'.$i.'" class="noedit" readonly></input>';
        
        echo '<label for="locatorname'.$i.'">Locator User Name '.$i.':</label>';
        echo '<input type="text" value="'.$locatorname.'" id="locatorname'.$i.'" class="noedit" readonly></input>';
        
        echo '<label for="imei'.$i.'">IMEI'.$i.':</label>';
        echo '<input type="text" value="'.$imei.'" id="imei'.$i.'" class="noedit" readonly></input>';
    ...
    while($sql2->fetch()){
       // echo '<p>';
        ...
        echo '<input type="text" value="'.$simimei.'" id="imei'.$y.'" class="noedit" readonly></input>';
