 ...
 */
    ...
    $sql1 = $conn->prepare('SELECT DISTINCT productmodel.productmodel,productmodelmaster.productmodelname FROM productmodel INNER JOIN invoices ON productmodel.invNum=invoices.invNum INNER JOIN customers ON customers.frstInv=invoices.frstInv INNER JOIN productmodelmaster ON productmodelmaster.productmodelid=productmodel.productmodel WHERE customers.frstInv=? AND invoices.status=?');
